...
import { AppState, Tag, Mood, Mantra, Quality, Routine, JournalEntry, User, Alarm } from '../types';
...
const INITIAL_STATE: AppState = {
  user: { firstName: '', age: null, gender: null, avatar: null },
...
export function useRoutineByTag(tag: Tag | null) {
  const { state } = useApp();
  if (!tag) return state.routines[0];
  return state.routines.find(r => r.tag === tag) || state.routines[0];
}
